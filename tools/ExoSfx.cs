    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                // Startup rollback: the new build could install cleanly and still crash on
                // launch. Stamp a pending marker, then give the app 25s to write the
                // heartbeat Program.cs emits after first frame. If it never does, restore
                // the .old-* backup so the user is back on the version that worked.
                string pendingMarker = Path.Combine(root, "app.pending-start");
                string heartbeatMarker = Path.Combine(installDir, "app.started");
                try
                {
                    Directory.CreateDirectory(root);
                    File.WriteAllText(pendingMarker, installedVersion ?? "");
                }
                catch { }
                ...
                // Only the rename-promote path keeps a backup worth restoring. Find the
                // newest .old-* sibling left by ReplaceDirectory.
                bool rollbackAvailable = false;
                string backupDir = null;
                try
                {
                    var oldBackups = Directory.GetDirectories(root, Path.GetFileName(installDir) + ".old-*");
                    if (oldBackups.Length > 0)
                    {
                        Array.Sort(oldBackups, StringComparer.Ordinal);
                        backupDir = oldBackups[oldBackups.Length - 1];
                    }
                    rollbackAvailable = backupDir != null && Directory.Exists(backupDir);
                }
                catch { /* no backup */ }
                DateTime heartbeatDeadline = DateTime.UtcNow.AddSeconds(25);
                bool started = false;
                while (DateTime.UtcNow < heartbeatDeadline)
                {
                    if (File.Exists(heartbeatMarker)) { started = true; break; }
                    Thread.Sleep(400);
                }
                try { File.Delete(pendingMarker); } catch { }

                if (!started && rollbackAvailable)
                {
                    Log("New build did not report a successful start; restoring previous version.");
                    StopOtherExo();
                    StopExoWebViews();
                    Thread.Sleep(600);
                    try
                    {
                        ...
                        Log("Previous version restored from " + backupDir);
                        Process.Start(new ProcessStartInfo
                        {
                            FileName = Path.Combine(installDir, ExeName),
                            WorkingDirectory = installDir,
                            UseShellExecute = true
                        });
                    }
                    ...
                    {
                        Log("Startup rollback failed: " + rollbackEx.Message +
                            " (previous version remains at " + backupDir + ")");
                    }
                }
                else if (!started)
                {
                    // No backup (first install or in-place) — leave the pending marker so a
                    // future launch can still clear it, and let the app's own recovery run.
                    Log("No startup heartbeat within 25s and no .old backup to restore; leaving installed build in place.");
                }
                else
                {
                    Log("New build confirmed started (heartbeat present).");
                }
