...
Expect("pwsh candidate order: Preview first",
    IndexAfter(optimizerText, @"PowerShell\7-preview\pwsh.exe", 0) >= 0 &&
...
Expect("memory guard uses Windows priority and passes thrash-free classifier",
    SteamLogic.IsMemoryGuardText(helperExpanded) &&
    helperExpanded.Contains("SetProcessInformation", StringComparison.Ordinal) &&
    helperExpanded.Contains("SetPowerThrottled", StringComparison.Ordinal) &&
    ...
    !helperExpanded.Contains("SetProcessWorkingSetSize", StringComparison.Ordinal) &&
...
// --- Native apply: no levers that cannot reach anything ---
var nativePath = Path.Combine(repo, "Exo", "Services", "SteamNativeApply.cs");
Expect("SteamNativeApply.cs present", File.Exists(nativePath));
if (File.Exists(nativePath))
{
    var native = File.ReadAllText(nativePath);

    // FSO only engages for a process that presents a fullscreen swapchain. steamwebhelper
    // is a windowed CEF renderer, so stamping it was a write that always reported ok and
    // could never take effect. steam.exe keeps the flag — Big Picture is real fullscreen.
    //
    // Anchored by source-order index, never by a multi-line literal: Windows CI checks the
    // repo out CRLF while this assertion is authored LF, so any matched string containing
    // a newline passes locally and fails in CI.
    var fsoStart = native.IndexOf("private static NativeApplyStep SetClientFso", StringComparison.Ordinal);
    var fsoEnd = native.IndexOf("public static string BuildMemoryGuardBody", StringComparison.Ordinal);
    Expect("SetClientFso body located", fsoStart >= 0 && fsoEnd > fsoStart);
    if (fsoStart >= 0 && fsoEnd > fsoStart)
    {
        var fsoBody = native[fsoStart..fsoEnd];
        Expect("FSO targets steam.exe, not every client exe",
            fsoBody.Contains("Path.Combine(steamPath, \"steam.exe\")", StringComparison.Ordinal)
            && fsoBody.Contains("webhelper skipped", StringComparison.Ordinal));
        Expect("FSO does not blanket-stamp the CEF renderer",
            !fsoBody.Contains("foreach (var exe in ClientExeTargets(steamPath))", StringComparison.Ordinal)
            || fsoBody.Contains("DeleteValue(stale", StringComparison.Ordinal));
        Expect("stale webhelper FSO stamps are cleared on re-apply",
            fsoBody.Contains("DeleteValue(stale", StringComparison.Ordinal));
    }

    // GPU routing is the shared policy, not a second copy of the topology probe.
    Expect("Steam GPU routing uses the shared policy",
        native.Contains("GpuTopology.RouteBrowserUi", StringComparison.Ordinal)
        && native.Contains("GpuTopology.IsHybrid()", StringComparison.Ordinal));
    Expect("Steam does not keep its own hybrid-detection copy",
        !native.Contains("private static bool IsHybridGraphics()", StringComparison.Ordinal));

    // High-performance is correct for real game EXEs and wrong for the client's own
    // Chromium UI. Assert the split by scoping to each method rather than banning the
    // string outright, which would forbid the one place it genuinely belongs.
    var routeStart = native.IndexOf("private static NativeApplyStep SetGpuRouting", StringComparison.Ordinal);
    var routeEnd = native.IndexOf("private static NativeApplyStep SetClientFso", StringComparison.Ordinal);
    Expect("SetGpuRouting body located", routeStart >= 0 && routeEnd > routeStart);
    if (routeStart >= 0 && routeEnd > routeStart)
    {
        Expect("client CEF is never routed to the discrete GPU",
            !native[routeStart..routeEnd].Contains("GpuPreference=2", StringComparison.Ordinal));
    }
    // Library games go to the discrete GPU - through the shared policy, like everything else.
    //
    // This used to pin the literal `gpu?.SetValue(exe, "GpuPreference=2;", ...)` line, which
    // asserted one implementation rather than the behaviour: the library sweep wrote that value
    // unconditionally, with no topology check at all, so a single-GPU desktop collected one
    // inert stamp per installed game (51 of them on a real machine with one adapter, which is
    // precisely what GpuTopology's type comment says the policy exists to prevent).
    // RouteGameHighPerf writes the identical value when the machine is hybrid and clears it when
    // it is not. Assert the routing call rather than the register write - and keep asserting the
    // split that actually matters, that the client's Chromium UI never gets high-performance.
    Expect("library game EXEs are routed to the discrete GPU through the shared policy",
        native.Contains("GpuTopology.RouteGameHighPerf", StringComparison.Ordinal));
    Expect("the library sweep does not hand-roll its own GpuPreference write",
        !native.Contains("gpu?.SetValue(exe, \"GpuPreference=2;\"", StringComparison.Ordinal));
}
...
// The app's Steam tile comes from NativeLiveDetect, NOT from Exo-Steam-Detect.ps1. The
// library-GPU rule was once corrected in the PowerShell detect alone, and because that
// script is shipped, hashed, gate-tested and CI-run, the fix looked complete while the row
// users actually see stayed red on every single-GPU desktop forever. Pin the live one.
{
    var livePath = Path.Combine(FindRepoRoot(), "Exo", "Services", "NativeLiveDetect.cs");
    if (File.Exists(livePath))
    {
        var live = File.ReadAllText(livePath);
        var gpuFn = live.IndexOf("LiveSteamLibraryGpu(string steamPath)", StringComparison.Ordinal);
        var body = gpuFn >= 0 ? live[gpuFn..] : "";
        Expect("live Steam library-GPU detect asks the same topology question Apply asks",
            body.Contains("GpuTopology.IsHybrid()", StringComparison.Ordinal));
        // On one adapter Apply DELETES the preference, so applied means absent, not stamped.
        Expect("single-GPU machines read a cleared GPU preference as applied",
            body.Contains("gpu?.GetValue(exe) is null", StringComparison.Ordinal));
    }
}
