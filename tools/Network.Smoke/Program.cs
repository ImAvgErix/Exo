...
// Host gaming stack (MMCSS / HAGS / Game Mode / Win32) is Windows-owned — Internet never writes it.
Expect("scripts leave MMCSS alone (Windows owns host gaming stack)",
    ...
    !latScript.Contains("Win32PrioritySeparation", StringComparison.Ordinal) &&
    !latScript.Contains("HwSchMode", StringComparison.Ordinal) &&
    !latScript.Contains("AutoGameModeEnabled", StringComparison.Ordinal));
// Network-owned: remove the old 20% QoS reserved-bandwidth tax.
...
Expect("experimental still leaves Windows host stack alone",
    ...
    !latExp.Contains("Win32PrioritySeparation", StringComparison.Ordinal));
...
// Post-apply verification: writing a setting and having it TAKE are different
// things. The apply must re-read the live stack and report per-lever truth, and
// must specifically catch the case where DSCP policies exist but the NLA switch
// is missing (Windows then ignores the marking entirely off-domain).
{
    Expect("apply re-reads TCP state after applying",
        latScript.Contains("Get-NetTCPSetting -SettingName 'Internet'", StringComparison.Ordinal)
        && latScript.Contains("Report 'verify'", StringComparison.Ordinal));
    Expect("apply verifies DSCP marking is actually honoured",
        latScript.Contains("Report 'verify-dscp'", StringComparison.Ordinal)
        && latScript.Contains("Do not use NLA", StringComparison.Ordinal));
    Expect("DSCP NLA switch is set so home networks honour the marking",
        latScript.Contains("New-ItemProperty -LiteralPath $qosRoot -Name 'Do not use NLA'", StringComparison.Ordinal));
    var repair = NetworkApplyScriptBuilder.BuildRepair();
    Expect("repair restores the NLA switch to its pre-Exo state",
        repair.Contains("qosDoNotUseNla", StringComparison.Ordinal)
        && repair.Contains("Do not use NLA", StringComparison.Ordinal));
}

// BOTH media must be optimized. A network optimizer that only tunes Ethernet is
// half a product — plenty of gaming rigs are on Wi-Fi, and Wi-Fi's biggest
// latency levers (radio power-save, MIMO power save, uAPSD, packet coalescing,
// band preference) do not exist on Ethernet at all.
// (No newline inside any matched literal: Windows CI checks out CRLF.)
{
    Expect("apply tunes BOTH Ethernet and Wi-Fi adapters",
        latScript.Contains("Test-IsWifiAdapter", StringComparison.Ordinal)
        && latScript.Contains("$kind = $(if ($isWifi) { 'Wi-Fi' } else { 'Ethernet' })", StringComparison.Ordinal));

    // Wi-Fi radio power saving is the single biggest Wi-Fi latency lever.
    foreach (var wifiLever in new[] { "PowerSaveMode", "MIMOPowerSaveMode", "uAPSDSupport", "UltraLowPowerMode" })
    {
        Expect("wifi lever applied: " + wifiLever,
            latScript.Contains(wifiLever, StringComparison.Ordinal));
    }

    // Device power-down is what makes a Wi-Fi link stutter under load; 24 =
    // "don't let the machine turn this adapter off".
    Expect("adapter power-down disabled (PnPCapabilities)",
        latScript.Contains("PnPCapabilities -Value 24", StringComparison.Ordinal));

    // Shared levers must reach every physical adapter, not just Ethernet.
    Expect("interrupt moderation + offloads applied per adapter (both media)",
        latScript.Contains("Set-Adv $n '*InterruptModeration'", StringComparison.Ordinal)
        && latScript.Contains("Get-ExoPhysicalAdapters", StringComparison.Ordinal));

    // Wi-Fi must never be collateral damage of an Ethernet-preferred apply.
    // Repair re-enables ANY disabled physical adapter unconditionally, which is
    // stronger than matching Wi-Fi by name (the old heuristic missed USB/odd
    // vendor names).
    Expect("repair re-enables any disabled physical adapter (Wi-Fi included)",
        repairScript.Contains("Get-NetAdapter -Physical", StringComparison.Ordinal)
        && repairScript.Contains("Enable-NetAdapter -Name $a.Name", StringComparison.Ordinal));
}
