...
static class Program
{
    ...
    static int ScanSettingIds()
    {
        ...
        File.WriteAllText(@"C:\Users\Erix\OptiHub\tools\OptiHub.NvCplScale\setting-hits.txt",
    ...
    static int ApplyAll(string[] args)
    {
        ...
        if (!File.Exists(hitFile))
            hitFile = @"C:\Users\Erix\OptiHub\tools\OptiHub.NvCplScale\setting-hits.txt";
        ...
        if (nv != null)
        {
            Console.WriteLine("[NvCplScale] Running OptiHub.NvDisplay --apply");
    ...
    static string? FindNvDisplay()
    {
        foreach (var c in new[]
        {
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
                @"OptiHub\scripts\Nvidia\tools\OptiHub.NvDisplay.exe"),
