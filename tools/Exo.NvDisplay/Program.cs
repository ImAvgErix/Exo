...
static class Program
{
    ...
    static int Main(string[] args)
    {
        ...
        var gpuStatus = normalizedArgs.Any(a => a is "--gpu-status" or "/gpu-status");
        var gpuApply = normalizedArgs.Any(a => a is "--gpu-apply" or "/gpu-apply");
        var gpuSnapshotRaw = GetArgValue(rawArgs, "--gpu-snapshot") ?? GetArgValue(rawArgs, "/gpu-snapshot");
        var gpuRestoreRaw = GetArgValue(rawArgs, "--gpu-restore") ?? GetArgValue(rawArgs, "/gpu-restore");
        var drsApplyRaw = GetArgValue(rawArgs, "--drs-apply") ?? GetArgValue(rawArgs, "/drs-apply");
        var drsStatusRaw = GetArgValue(rawArgs, "--drs-status") ?? GetArgValue(rawArgs, "/drs-status");
        ...
        var knownPrefixes = new[]
        {
            ...
            "--gpu-status", "/gpu-status", "--gpu-apply", "/gpu-apply",
            "--gpu-snapshot", "/gpu-snapshot", "--gpu-restore", "/gpu-restore",
            "--drs-apply", "/drs-apply", "--drs-status", "/drs-status"
        };
        ...
        var drsOperation = drsBackupRaw is not null || drsRestoreRaw is not null
                           || drsApplyRaw is not null || drsStatusRaw is not null;
        // GPU power/thermal/cooler work is its own mode. Without excluding it here the default
        // "no recognised flag means --apply" fallback below would run the whole display apply
        // as a side effect of asking for GPU status.
        var gpuOperation = gpuStatus || gpuApply || gpuRestoreRaw is not null;
        var apply = !statusOnly && !listColor && !listDisplays && !panelMutate && !getVibrance
                    && !drsOperation && !gpuOperation;
        ...
        if (normalizedArgs.Any(a => a is "--help" or "-h" or "/?"))
        {
            ...
            Console.WriteLine("  --gpu-status         Report power / thermal ceilings and cooler control mode");
            Console.WriteLine("  --gpu-apply [--gpu-snapshot PATH]  Raise power + thermal to the board's own maximum");
            Console.WriteLine("  --gpu-restore PATH   Put power + thermal back to the recorded pre-Exo values");
            Console.WriteLine("  --drs-apply PATH.nip Apply a profile pack through NVAPI (no Profile Inspector)");
            Console.WriteLine("  --drs-status PATH.nip  Report which of a pack's settings are live");
            return 0;
        }
        ...
        // GPU power / thermal / cooler. Handled before the display pipeline because it shares
        // none of it — different NVAPI surface, different failure modes, and it must not drag
        // a display re-apply along behind it.
        // DRS packs applied natively. Handled before the display pipeline for the same reason
        // the GPU power block is: different NVAPI surface, and it must not drag a display
        // re-apply along behind it.
        if (drsApplyRaw is not null) return Exo.NvDisplay.GpuDrs.Apply(drsApplyRaw);
        if (drsStatusRaw is not null) return Exo.NvDisplay.GpuDrs.Status(drsStatusRaw);

        if (gpuOperation)
        {
            if (gpuRestoreRaw is not null) return Exo.NvDisplay.GpuPower.Restore(gpuRestoreRaw);
            if (gpuApply) return Exo.NvDisplay.GpuPower.Apply(gpuSnapshotRaw);
            return Exo.NvDisplay.GpuPower.Status();
        }
        ...
        try
        {
            ...
            {
                // Keep current resolution. Primary -> max Hz; secondary per policy
                // (EXO_SECONDARY_REFRESH, default keep - never silently downgrade a
                // second high-refresh monitor).
            ...
            if (apply)
            {
                ...
                // Status-only runs never entered the apply block above, so modesOk was still
                // sitting at its `true` initializer and the refresh policy went unchecked —
                // a monitor running 120 Hz on a 165 Hz panel reported as fully applied.
                // Detect has to measure, not assume.
                if (!apply && nvidiaGdiNames.Count > 0)
                    modesOk = VerifyTargetRefreshModes(nvidiaGdiNames);
                ...
                // Hard gate: refresh AND color AND scaling. Color is read back from the driver, so a
                // registry stamp must never authorize skipping it -- that is what let a monitor sit on
                // limited-range CEA while the run reported "Full RGB applied".
                var ok = modesOk && colorOk && scalingOk;
                try
                {
                    var json = JsonSerializer.Serialize(new
                    {
            ...
            else
            {
                ...
                // Same hard gate as apply: refresh AND color AND scaling, every term read back live.
                var ok = refreshOk && colorOk && scalingOk;
                try
                {
                    var json = JsonSerializer.Serialize(new
                    {
    ...
    /// <summary>
    /// Highest bit depth the link will actually carry, probed high → low.
    ///
    /// This used to put the *current* depth first in the probe order, which meant an
    /// 8-bit link that also supports 10-bit returned 8 on the very first probe and could
    /// never climb — "highest bit depth" was unreachable for exactly the displays that
    /// had headroom. 12-bit is still only considered when the display is already there,
    /// because forcing it on a panel that merely reports support is how you get a link
    /// that trains down to YCbCr422 behind your back.
    /// </summary>
        ...
        bool requireGpuScale = true)
    {
        try
        {
            ...
            {
                // Having inspected nothing proves nothing. Only a caller that requires
                // nothing may pass here; anything else has to be re-applied.
                Console.WriteLine("[NVTweak] Verify: no device keys present - registry proves nothing");
                return !requireColor && !requireGpuScale;
            }

            // Every key is checked. NVIDIA names these keys from an EDID-derived hash
            // ("1089131374-0"), never from an NVAPI display id ("2147881088"), so filtering
            // them against the active display ids discarded all of them on every machine and
            // then reported "verified". ApplyNvtweakRegistry stamps all keys, so verify all keys.
            ...
            if (!checkedAny)
            {
                Console.WriteLine("[NVTweak] Verify: no device key could be opened - registry proves nothing");
                return !requireColor && !requireGpuScale;
            }
