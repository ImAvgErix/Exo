...
// PowerShell 7 host: Preview preferred, stable accepted; never uninstall Preview
var discOptText = File.ReadAllText(applyFiles[0]);
Expect("pwsh host classifier present",
...
Expect("pwsh candidate order: Preview first",
    discOptText.IndexOf(@"PowerShell\7-preview\pwsh.exe", StringComparison.OrdinalIgnoreCase) >= 0 &&
...
Expect("pwsh install hint prefers Preview winget id",
...
Expect("legacy preview-only host asserts removed",
...
//
// These titles are display copy, and copy gets rewritten: the installed-branch row
// was renamed "Complete client debloat" -> "Slim client cleanup" in 3.13.11 and this
// gate was never updated. Because the stale title is only used on a machine that
// actually has Discord installed - never on CI, which takes the install branch - the
// gate went red on real user PCs and stayed green everywhere it was watched, all the
// way through the 4.4.1 release. So assert the title still exists in the detect source
// before running it: a future rename now fails here, loudly and with the reason,
// instead of surfacing as a mystery 0/5 on somebody's gaming rig.
...
if (File.Exists(detectPs1))
{
    ...
    var expectedRow = discordInstalled ? "Slim client cleanup" : "Discord install";
    Expect($"detect source still emits the '{expectedRow}' row title",
        File.ReadAllText(detectPs1).Contains(expectedRow, StringComparison.Ordinal),
        "row display copy was renamed - update expectedRow in Discord.Smoke to match");
    ...
    try
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            ...
            // Drain both pipes concurrently. Reading stdout to the end first blocks
            // until the child closes it, so a child that fills the 4 KB stderr buffer
            // meanwhile deadlocks: it waits for us to read stderr, we wait for it to
            // finish stdout. Start both reads, then wait.
            var outTask = pl.StandardOutput.ReadToEndAsync();
            var errTask = pl.StandardError.ReadToEndAsync();
            pl.WaitForExit(90000);
            var liveOut = outTask.GetAwaiter().GetResult();
            var liveErr = errTask.GetAwaiter().GetResult();
...
// --- The C# kit-version fallback must not drift from the shipped kit ---
// It is only used when Scripts/Discord/VERSION is missing, but a literal that has to
// match a file and is checked by nobody is a literal that goes stale. Turn it into a
// build failure instead of a slow rot.
{
    var versionFile = Path.Combine(repoRoot, "Exo", "Scripts", "Discord", "VERSION");
    var settingsCs = Path.Combine(repoRoot, "Exo", "Models", "AppSettings.cs");
    var serviceCs = Path.Combine(repoRoot, "Exo", "Services", "SettingsService.cs");
    Expect("Discord kit VERSION file present", File.Exists(versionFile));
    if (File.Exists(versionFile) && File.Exists(settingsCs) && File.Exists(serviceCs))
    {
        var kit = File.ReadAllText(versionFile).Trim();
        Expect($"AppSettings fallback matches shipped kit ({kit})",
            File.ReadAllText(settingsCs).Contains($"\"{kit}\"", StringComparison.Ordinal));
        Expect($"SettingsService fallback matches shipped kit ({kit})",
            File.ReadAllText(serviceCs).Contains($"\"{kit}\"", StringComparison.Ordinal));
    }
}

// --- Voice QoS must actually reach the wire ---
// Windows silently drops policy-based DSCP marking on a non-domain network unless the
// "Do not use NLA" switch is set. Writing the policy and reading the keys back proves
// the policy exists, not that anything is marked — this module reported success for a
// no-op on every home network until that switch was added.
{
    var debloat = Path.Combine(repoRoot, "Exo", "Scripts", "Discord", "kit", "lib", "40-DebloatWindows.ps1");
    var repair = Path.Combine(repoRoot, "Exo", "Scripts", "Discord", "Exo-Discord-Repair.ps1");
    Expect("QoS source files present", File.Exists(debloat) && File.Exists(repair));
    if (File.Exists(debloat) && File.Exists(repair))
    {
        var d = File.ReadAllText(debloat);
        var r = File.ReadAllText(repair);

        Expect("apply sets the QoS NLA switch",
            d.Contains("Set-ExoQosNlaSwitch", StringComparison.Ordinal)
            && d.Contains("'Do not use NLA'", StringComparison.Ordinal));
        // Retired switches steal GPU/CPU while gaming — must stay stripped.
        var profileJson = Path.Combine(repoRoot, "Exo", "Scripts", "Discord", "kit", "profiles", "discord.json");
        Expect("discord.json present for retired-switch check", File.Exists(profileJson));
        if (File.Exists(profileJson))
        {
            var pj = File.ReadAllText(profileJson);
            Expect("profile does not re-add disable-renderer-backgrounding",
                !pj.Contains("disable-renderer-backgrounding", StringComparison.OrdinalIgnoreCase));
            Expect("profile does not re-add disable-backgrounding-occluded-windows",
                !pj.Contains("disable-backgrounding-occluded-windows", StringComparison.OrdinalIgnoreCase));
            Expect("profile keeps disable-background-timer-throttling",
                pj.Contains("disable-background-timer-throttling", StringComparison.OrdinalIgnoreCase));
        }
        Expect("apply actively retires renderer-backgrounding switches",
            d.Contains("disable-renderer-backgrounding", StringComparison.Ordinal)
            && d.Contains("retired", StringComparison.OrdinalIgnoreCase));
        Expect("NLA switch is set before the DSCP policies are written",
            d.IndexOf("[void](Set-ExoQosNlaSwitch)", StringComparison.Ordinal) > 0
            && d.IndexOf("[void](Set-ExoQosNlaSwitch)", StringComparison.Ordinal)
               < d.IndexOf("QoS DSCP 46 policy active", StringComparison.Ordinal));
        // This originally asserted against Test-DiscordVoiceQosApplied in the kit lib —
        // which was never called by anything. The assertion passed while checking dead
        // code. The live path is Exo-Discord-Detect.ps1, so that is what gets asserted,
        // and the dead duplicate has been deleted so a fix cannot land on it again.
        Expect("live detect script present", File.Exists(detectPs1));
        if (File.Exists(detectPs1))
        {
            var det = File.ReadAllText(detectPs1);
            Expect("live detect does not pass QoS on policy keys alone",
                det.Contains("'Do not use NLA'", StringComparison.Ordinal)
                && det.Contains("if (-not $nlaOk) { $qosOk = $false }", StringComparison.Ordinal));
        }
        Expect("the dead duplicate QoS classifier is gone",
            !d.Contains("function Test-DiscordVoiceQosApplied", StringComparison.Ordinal));
        Expect("pre-Exo NLA value is captured in the before-snapshot",
            d.Contains("QosDoNotUseNla   = $qosNla", StringComparison.Ordinal));
        Expect("first-seen NLA value survives a re-apply",
            d.Contains("QosDoNotUseNla  = $priorNla", StringComparison.Ordinal));

        // The switch is machine-wide and the Internet optimizer needs it too. Discord
        // Repair must not clear it out from under a module the user did not touch.
        Expect("Repair leaves the shared NLA switch alone when other Exo QoS policies remain",
            r.Contains("still need it", StringComparison.Ordinal)
            && r.Contains("leftover", StringComparison.Ordinal));
        Expect("Repair restores a recorded pre-Exo NLA value",
            r.Contains("QosDoNotUseNla", StringComparison.Ordinal)
            && r.Contains("was not set before Exo", StringComparison.Ordinal));
    }
}
