...
var hags = ExoActionSafety.FilterActions(
[
    new ExoToolAction { ToolId = "module.windows.apply" },
    new ExoToolAction { ToolId = "display.hagsMpoVrrMatrix" }
], out var hagsRejected);
Expect("HAGS bundle keeps windows", hags.Count == 1 && hags[0].ToolId == "module.windows.apply");
Expect("HAGS bundle rejects matrix thrash", hagsRejected.Count == 1);

var braveWv = ExoActionSafety.FilterActions(
[
    new ExoToolAction { ToolId = "browser.braveOnly" },
    new ExoToolAction { ToolId = "webview2.uninstall" }
], out var braveRejected);
Expect("Brave×WebView2 denylist", braveWv.Count == 1 && braveWv[0].ToolId == "browser.braveOnly");
Expect("Brave×WebView2 rejects uninstall", braveRejected.Any(r => r.Contains("webview2", StringComparison.OrdinalIgnoreCase)));

var spooler = ExoActionSafety.FilterActions(
[
    new ExoToolAction { ToolId = "print.spoolerGate", Params = { ["printerCount"] = "2" } },
    new ExoToolAction { ToolId = "power.exoCompetitive" }
], out var spoolerRejected);
Expect("spooler×printers rejects gate", spooler.Count == 1 && spooler[0].ToolId == "power.exoCompetitive");
Expect("spooler×printers reason", spoolerRejected.Any(r => r.Contains("printers", StringComparison.OrdinalIgnoreCase)));

var islc = ExoActionSafety.FilterActions(
[
    new ExoToolAction { ToolId = "memory.islc" },
    new ExoToolAction { ToolId = "memory.compressionOff" }
], out var islcRejected);
Expect("ISLC×compression keeps first", islc.Count == 1 && islc[0].ToolId == "memory.islc");
Expect("ISLC×compression rejects second", islcRejected.Count == 1);
