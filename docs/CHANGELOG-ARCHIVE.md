...
Releases 4.2.9 and earlier. The current changelog - and the one shown inside the app -
...
**Three game-config bugs that were failing invisibly.**

- **CS2 and Apex never actually went borderless.** Exo wrote two contradictory
  display settings at once, so the games quietly stayed in exclusive fullscreen
  no matter what it reported. Both now write the correct combination.
- **If you'd ever set a game config read-only, Exo silently did nothing to it.**
  That's common advice (games rewrite these files on launch), and it made the
  write fail in a way that was swallowed and reported as success. The lock is now
  cleared before writing.
- **Repair could report success while restoring nothing.** Restoring over a
  read-only config threw an error that was swallowed, so you'd be told your
  settings were back when they weren't. Fixed, and Repair now leaves files
  exactly as it found them.
- CS2's video config is also locked after writing, because CS2 rewrites it on
  every launch -- without that, the tweak was gone before you got into a match.

## 4.2.8

**The network module now proves its work -- and a fix that was silently doing nothing.**

- **Game/voice traffic priority was being ignored on home networks.** Exo writes
  QoS rules that tag your game and voice packets as high priority, but Windows
  only honours them when a specific switch is set -- and on any non-domain (read:
  home) network it wasn't. The rules existed, looked applied, and never marked a
  single packet. Now set correctly, and Repair puts it back exactly as it was.
- **It checks its own work.** After applying, Exo re-reads the live network stack
  and reports each setting as confirmed or not-applied, instead of assuming the
  write landed. If the priority tagging can't actually take effect, it now says
