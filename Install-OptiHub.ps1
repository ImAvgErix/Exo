...
Invoke-WebRequest -Uri $asset.browser_download_url -OutFile $sfx -UseBasicParsing -Headers @{ 'User-Agent' = 'OptiHub-Installer/2.0' } -TimeoutSec 300
...
if ($asset.size -and $downloaded.Length -ne [long]$asset.size) {
    ...
    throw "Downloaded OptiHub.exe has the wrong size ($($downloaded.Length); expected $($asset.size))."
}
...
if ($expectedDigest -notmatch '^sha256:[0-9a-fA-F]{64}$') {
    Remove-Item -LiteralPath $sfx -Force -ErrorAction SilentlyContinue
    throw 'GitHub did not provide a valid SHA-256 digest for OptiHub.exe.'
}
...
if ($actualHash -ne $expectedHash) {
    Remove-Item -LiteralPath $sfx -Force -ErrorAction SilentlyContinue
    throw 'Downloaded OptiHub.exe failed its SHA-256 integrity check.'
}
