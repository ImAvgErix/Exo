...
# ============================================================================
# ALWAYS (both paths): Exo-Net-DSCP-* QoS policies + the NLA switch. These are
# KEYS the apply creates, not values the snapshot records, so the snapshot-driven
# restore above never touches them — this block used to live only in the fallback,
# which meant every normal Repair (snapshot present) left the DSCP policies and
# the machine-wide 'Do not use NLA' switch behind forever, and the line reading
# $snap.qosDoNotUseNla sat in the one branch where $snap is guaranteed null.
# ============================================================================
try {
  ...
  if (Test-Path -LiteralPath $qosRoot) {
    ...
    # 'Do not use NLA' is machine-wide and SHARED. Discord's voice policies and Steam's
    # client/library policies are dropped by Windows off-domain without it, so reverting it
    # here unconditionally meant repairing Internet silently neutered two other modules the
    # user had not asked us to touch - no message, and Steam's detect row would still read
    # green because it only checks that its policy key exists. A real machine here carried
    # 50 non-Internet Exo QoS policies that this would have quietly killed.
    #
    # Discord's Repair already got this right; this is the same rule, ported.
    $otherExoQos = @(Get-ChildItem -LiteralPath $qosRoot -ErrorAction SilentlyContinue |
      Where-Object { ($_.PSChildName -like 'Exo-*' -or $_.PSChildName -like 'Exo *') -and
                     $_.PSChildName -notlike 'Exo-Net-DSCP-*' })
    if ($otherExoQos.Count -gt 0) {
      Log ("[QoS] Do not use NLA left in place - " + $otherExoQos.Count + " other Exo QoS policy(ies) still need it")
    } else {
      # Restore to exactly its pre-Exo state (Apply sets it to 1 so DSCP marking is
      # honoured off-domain). Absent before => remove it now.
      $nlaPrev = $null
