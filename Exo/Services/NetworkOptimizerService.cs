...
public sealed class NetworkOptimizerService
{
        ...
        CancellationToken ct = default)
    {
        ...
        // A cancellation the caller asked for is not a failure, and it must reach the caller as
        // one. The generic catch below would turn Stop into (false, "The operation was
        // canceled."), which the bridge rethrows as InvalidOperationException and the orb
        // announces as the run having broken. Rethrow so Stop reads as Stop.
        ...
        CancellationToken ct = default)
    {
        ...
        // An internal timeout means "no usable benchmark", which is null. A cancellation the
        // caller asked for means Stop, and returning null for it made the bridge throw
        // "Connection quality test could not finish (need a working internet path)" — blaming
        // the user's connection for their own Stop.
    ...
    public async Task<NetworkSnapshot> ProbeAsync(CancellationToken ct = default)
    {
        ...
        // Same rule as Apply and Repair: a Stop is not a degraded probe. Detect callers pass
        // CancellationToken.None, so the filter never fires for them and nothing changes there.
        catch (OperationCanceledException) when (ct.IsCancellationRequested)
        {
            throw;
        }
...
""", ct).ConfigureAwait(false);
    ...
    public bool MatchesPreset(NetworkSnapshot snap, NetworkPreset preset)
    {
        ...
        if (!NetworkLogic.CongestionMatches(snap.CongestionProvider)) return false;
        ...
        if (snap.ActivePreset == preset && !snap.Media.NicOk &&
            ...
            return false;
        // Adapter bindings (QoS+IPv4+IPv6) when probe reported them
        if (snap.ActivePreset == preset && !snap.Media.AdapterBindingsOk &&
            snap.Media.AdapterBindingsHint is not ("-" or "" or null))
            return false;
        // Prefer-Ethernet metric path: when Ethernet is in use, its metric should be low
        if (snap.Media.EthernetInUse &&
            snap.Media.EthernetMetric is int m && m > 25)
            return false;
        // Feature rows: only fail match when most non-diagnostic rows are off — soft props
        // vary by NIC; hard fails already live in reportFailures.
        if (snap.Features is { Count: > 0 })
        {
            var list = snap.Features.Where(f =>
                !string.IsNullOrWhiteSpace(f.Title) &&
                !f.Title.Contains("diagnostic", StringComparison.OrdinalIgnoreCase) &&
                !f.Title.Contains("quality", StringComparison.OrdinalIgnoreCase) &&
                !f.Title.Contains("benchmark", StringComparison.OrdinalIgnoreCase)).ToList();
            if (list.Count >= 3 && list.Count(f => !f.IsOk) > list.Count / 2)
                return false;
        }
        return true;
    }
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // Incomplete live verify is not full success — report partial so the orb can surface it.
            return (false,
                $"Applied ({policy}). Live verification incomplete ({hint}). " +
                "Refresh Internet or re-apply after a moment; open Last apply for step detail.");
        }
        // A cancellation the caller asked for is not a failure, and it must reach the caller as
        // one. The generic catch below would turn Stop into (false, "The operation was
        // canceled."), which the bridge rethrows as InvalidOperationException and the orb
    ...
    private static async Task<string> RunCaptureAsync(string file, string args, CancellationToken ct)
    {
        try
        {
            ...
            // Drain stderr too: redirected but unread, a chatty tool fills the pipe
            // buffer and blocks before ever finishing stdout.
