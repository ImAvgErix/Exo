...
public static partial class SteamLogic
{
    ...
    public static bool IsMemoryGuardText(string? text)
    {
        ...
        // EcoQoS on non-foreground (library + in-game).
        if (!text.Contains("SetPowerThrottled($_.Id, ($_.Id -ne $foregroundPid))", StringComparison.Ordinal) &&
            ...
            return false;
        // Working-set thrash freezes modern CEF — ban EmptyWorkingSet AND SetProcessWorkingSetSize
        // (SoftReclaimWorkingSet was the same call under another name).
        foreach (var rawLine in text.Split('\n'))
        {
            ...
            if (line.Contains("SoftReclaimWorkingSet", StringComparison.Ordinal)) return false;
    ...
    public static readonly string[] RequiredApplyMarkers =
    {
        ...
        ".exo-bak",
        // PowerShell 7 host resolution (Preview preferred; never 5.1)
        "Get-ExoPwsh",
    ...
    public static readonly string[] ForbiddenApplyPatterns =
    {
        ...
        "'-silent'",
        // Retired hard-require-Preview-only APIs (Preview is preferred, not exclusive)
        "Assert-ExoPwshPreview",
        "Test-ExoIsPwshPreviewHost",
        // Doctor must never uninstall beta hosts
        "preview-pwsh-uninstall|ok",
        "winget uninstall --id Microsoft.PowerShell.Preview",
        "winget uninstall --id Microsoft.WindowsTerminal.Preview",
    };
