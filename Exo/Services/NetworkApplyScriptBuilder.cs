...
# things: a template can silently reject a value, a policy can be overridden, and
# DSCP marking is ignored entirely off-domain. Re-read the live stack and report
# each lever as verified / not-applied so the UI can only claim what is true.
# ---------------------------------------------------------------------------
try {
  $vOk = 0; $vBad = 0
  $notes = New-Object System.Collections.Generic.List[string]

  # 1) TCP autotuning + congestion provider actually in effect on the Internet template.
  try {
    $liveTcp = Get-NetTCPSetting -SettingName 'Internet' -EA SilentlyContinue | Select-Object -First 1
    if ($liveTcp) {
      $atl = [string]$liveTcp.AutoTuningLevelLocal
      if ($atl -match '(?i)normal') { $vOk++ } else { $vBad++; [void]$notes.Add("autotuning=$atl") }
      $cp = [string]$liveTcp.CongestionProvider
      if ($cp -match '(?i)cubic') { $vOk++ } else { $vBad++; [void]$notes.Add("congestion=$cp") }
    } else { [void]$notes.Add('tcp template unreadable') }
  } catch { [void]$notes.Add('tcp read failed') }

  # 2) DSCP: policies present AND the NLA switch that makes Windows honour them on a
  #    non-domain network. Without the switch the policies exist but never mark a packet.
  try {
    $qosRoot = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\QoS'
    $dscpCount = 0
    if (Test-Path -LiteralPath $qosRoot) {
      $dscpCount = @(Get-ChildItem -LiteralPath $qosRoot -EA SilentlyContinue |
        Where-Object { $_.PSChildName -like 'Exo-Net-DSCP-*' }).Count
    }
    $nla = $null
    try { $nla = (Get-ItemProperty -LiteralPath $qosRoot -Name 'Do not use NLA' -EA SilentlyContinue).'Do not use NLA' } catch { }
    if ($dscpCount -gt 0 -and [string]$nla -eq '1') {
      $vOk++
      Report 'verify-dscp' 'ok' ("$dscpCount policy(ies) marking, NLA switch on")
    } elseif ($dscpCount -gt 0) {
      $vBad++
      Report 'verify-dscp' 'partial' ("$dscpCount policy(ies) written but 'Do not use NLA' is not set - Windows ignores DSCP off-domain")
    } else {
      Report 'verify-dscp' 'skip' 'no Exo DSCP policies (no target apps installed)'
    }
  } catch { Report 'verify-dscp' 'skip' 'QoS state unreadable' }

  if ($vBad -eq 0) {
    Report 'verify' 'ok' ("$vOk lever(s) confirmed live")
  } else {
    Report 'verify' 'partial' ("$vOk confirmed, $vBad did not take: " + ($notes -join '; '))
  }
} catch { Report 'verify' 'skip' 'post-apply verification unavailable' }
