...
public sealed class ScriptBundleService
{
    ...
    private static void SyncSharedLibFromBundled()
    {
        ...
        // Copy the full shared lib (ps1 helpers + RunHidden.vbs). Kits and Steam
        // native apply expect Exo.RunHidden.vbs beside the .ps1 helpers; a *.ps1-only
        // sync left the VBS missing and elevated apply refused on integrity.
        foreach (var file in Directory.EnumerateFiles(bundled, "*", SearchOption.TopDirectoryOnly))
        {
