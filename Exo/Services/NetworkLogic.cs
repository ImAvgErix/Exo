...
public static partial class NetworkLogic
{
    ...
    // "partial" belongs in this alternation because the apply script emits it: verify-dscp
    // reports partial when the policies were written but "Do not use NLA" is not set (Windows
    // then ignores DSCP marking off-domain, which is every home PC), and verify reports partial
    // when only some levers came back confirmed. Neither line matched, so both were dropped on
    // the floor - the exact two cases where the tuning did NOT fully take were the two the user
    // never heard about.
    [GeneratedRegex(@"EXO_REPORT:([A-Za-z0-9_\-\. ]+)\|(ok|fail|skip|partial)(?::([^\r\n]*))?", RegexOptions.IgnoreCase)]
