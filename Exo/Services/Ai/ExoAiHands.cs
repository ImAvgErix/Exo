...
public sealed class ExoAiHands
{
    ...
    private readonly NetworkOptimizerService _internet;
    ...
    public ExoAiHands(
        ...
        NetworkOptimizerService internet,
    ...
    private async Task<ExoToolResult> InternetAsync(IProgress<string>? progress, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var (ok, msg) = await _internet.ApplyPresetAsync(
                ...
                new NetworkApplyOptions { Experimental = _settings.Current.ExperimentalInternet, RestartEthernet = true },
    ...
    private async Task<ExoToolResult> DisplayMatrixAsync(IProgress<string>? progress, CancellationToken ct)
    {
        ...
        // HAGS + MPO clear (stock DWM — never OverlayTestMode=5) via Windows native Apply.
        progress?.Report("display: HAGS×MPO via Windows host stack (MPO flicker keys cleared)");
        var win = await ModuleNativeAsync("windows", progress, ct).ConfigureAwait(false);
        return new ExoToolResult
        {
            ...
            Success = win.Success,
            Status = win.Status,
            Message = "HAGS/MPO clear via Windows host (no OverlayTestMode=5): " + win.Message
        };
    }
    ...
    private Task<ExoToolResult> EcoQosAsync()
    {
        ...
        return Task.FromResult(new ExoToolResult
        {
            ...
            Message =
                ...
                "(non-FG Steam/Discord/Riot/Epic helpers; AC untouched)"
        });
    }
