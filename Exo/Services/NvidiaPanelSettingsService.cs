...
public sealed class NvidiaPanelSettingsService
{
    ...
    public void Save(NvidiaPanelSettings settings)
    {
        ...
        // Primary always goes to its highest refresh — that is the point of the module.
        settings.PrimaryRefresh = "max";

        // Secondary used to be stomped to "60" here on every save, so a second high-refresh
        // monitor was silently halved and the user could not opt out — a downgrade shipped
        // as an optimization, and invisible on a rig whose second panel is 60 Hz anyway.
        // Default is now "keep" (leave the user's choice alone); "max" and "60" stay
        // selectable, since dropping a secondary can still help on older mixed-refresh setups.
        if (string.IsNullOrWhiteSpace(settings.SecondaryRefresh))
            settings.SecondaryRefresh = "keep";
        File.WriteAllText(
            SettingsPath,
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            // Status has to verify against the SAME policy apply used, or detect and apply
            // disagree forever. Hardcoding "60" here meant a machine whose secondary is
            // legitimately left alone got measured against a target it was never given.
            var policy = Load();
            psi.Environment["EXO_PRIMARY_REFRESH"] = "max";
            psi.Environment["EXO_SECONDARY_REFRESH"] =
                string.IsNullOrWhiteSpace(policy.SecondaryRefresh) ? "keep" : policy.SecondaryRefresh;
            ...
            // Status gate: refresh AND color AND scaling. When the helper reports a checks block
            // its own "ok" must not override it -- the old "|| ok" was a second route by which a
            // failed color read still read as applied.
            if (el.TryGetProperty("checks", out _))
                ok = NvidiaDetectLogic.IsDisplayStatusOk(refreshOk, colorOk, scalingOk);
    ...
    public bool TryLaunchControlPanel(out string? error)
    {
        ...
        error = "NVIDIA Control Panel not found. Exo applies display settings through NVAPI without it — install NVIDIA's panel yourself if you want that UI, or use driver settings.";
        return false;
    }
