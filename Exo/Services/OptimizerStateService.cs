...
public sealed class OptimizerStateService
{
    ...
    /// Module names: "discord", "steam", "brave".
    /// </summary>
    ...
    public Task<OptimizerStateInfo> DetectSystemAsync(CancellationToken ct = default) =>
        Task.Run(() => NativeLiveDetect.DetectSystem(), ct);

    public Task<OptimizerStateInfo> DetectSpotifyAsync(CancellationToken ct = default) =>
        Task.Run(() => NativeLiveDetect.DetectSpotify(), ct);

    public Task<OptimizerStateInfo> DetectAmdAsync(CancellationToken ct = default) =>
        Task.Run(() => NativeLiveDetect.DetectAmd(), ct);
    ...
    private OptimizerStateInfo DetectDiscordHeuristic()
    {
        ...
        // Equicord AMOLED theme — only when the theme is actually selected/enabled.
        // A file on disk alone used to green this row while Discord still ran stock chrome.
        try
        {
    ...
    private static bool IsQosPolicyPresent(string policyName, string exe)
    {
        try
        {
            // Home networks drop DSCP marking unless "Do not use NLA"=1. Full PS detect
            // already requires this; the C# heuristic used to green inert policies.
            if (!IsQosNlaBypassEnabled()) return false;
    ...
    /// <summary>Windows only marks QoS off-domain when this policy is 1.</summary>
    private static bool IsQosNlaBypassEnabled()
    {
        try
        {
            using var key = Microsoft.Win32.Registry.LocalMachine.OpenSubKey(
            ...
            if (key is null) return false;
            var v = key.GetValue("Do not use NLA");
        ...
        bool fastOnly = false)
    {
        ...
        try
        {
            ...
            if (root.TryGetProperty("applyStatus", out var aps) && aps.ValueKind == JsonValueKind.String)
                extra["applyStatus"] = aps.GetString() ?? "";
            if (root.TryGetProperty("partialReasons", out var prs) && prs.ValueKind == JsonValueKind.Array)
                extra["partialReasons"] = string.Join(" ", prs.EnumerateArray()
    ...
    private OptimizerStateInfo DetectNvidiaHeuristic()
    {
        ...
        if (File.Exists(statePath))
        {
            try
            {
                ...
                // The "policy" string used to be consulted here and could only ever set
                // safePolicy back to true, which made the explicit boolean above pointless: the
                // optimizer writes policy='safe-drs-v2' as a constant regardless of the switch,
                // so the substring test for "safe"/"drs" matched on every machine ever written.
                // A state file saying safePolicy=false was read correctly and then discarded one
                // line later, and the NVIDIA tile described the Control-Panel-only module the
                // user was not running — displays "left as-is" while the same file recorded max
                // Hz, Full RGB and colour override, and no debloat row at all despite
                // debloatApplied=true. Removed rather than reordered: a heuristic that is
                // unconditionally true carries no information to fall back on.
                ...
                applyStatus = ReadString(root, "applyStatus");
                if (root.TryGetProperty("partialReasons", out var reasons) && reasons.ValueKind == JsonValueKind.Array)
                    partialReasons = string.Join(" ", reasons.EnumerateArray()
                        .Where(x => x.ValueKind == JsonValueKind.String)
                        .Select(x => x.GetString())
                        .Where(x => !string.IsNullOrWhiteSpace(x)));
            }
        ...
        // This is the fast heuristic used before the detect script runs — it has no live
        // NVAPI read, so it cannot know the real display state. It used to claim
        // "Manual in Control Panel" with active: true, a row that was green on every
        // machine forever. Unknown is reported as unknown; the script row replaces it.
        const string displayPending = "Checking refresh, colour range, and scaling…";
        if (safePolicy)
        {
            ...
            features.Add(MakeFeature("Display colors, refresh & scaling",
                "Left as-is by the Control-Panel-only policy.", true));
        ...
        else
        {
            ...
            features.Add(MakeFeature("Display colors, refresh & scaling", displayPending, false));
        }
        ...
        if (!string.IsNullOrWhiteSpace(applyStatus))
            extra["applyStatus"] = applyStatus!;
        if (!string.IsNullOrWhiteSpace(partialReasons))
            extra["partialReasons"] = partialReasons!;
