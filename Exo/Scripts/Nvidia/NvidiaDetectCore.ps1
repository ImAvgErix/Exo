...
function Test-ExoDisplayStatusOk {
    # Every term is required and every term is read back from the driver. The old gate let
    # RegistryOk short-circuit ColorOk while the registry check itself passed vacuously, so a
    # monitor could sit on limited-range CEA and still report "Full RGB applied".
    param([bool]$RefreshOk, [bool]$ColorOk, [bool]$ScalingOk)
    return [bool]($RefreshOk -and $ColorOk -and $ScalingOk)
}
