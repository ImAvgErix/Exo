...
function Get-ExoNvidiaPanelSettings {
    $defaults = [pscustomobject]@{
        primaryRefresh          = 'max'
        # 'keep' leaves a second high-refresh monitor alone. It used to default to '60',
        # which silently halved a 144/165 Hz secondary and was invisible to test on a rig
        # whose second panel maxes at 60 anyway. '60' and 'max' are still selectable.
        secondaryRefresh        = 'keep'
        fullRgb                 = $true
        gpuNoScaling            = $true
...
function Set-NvidiaStoreCplVirtualHive {
    ...
    try {
        ...
        # Read back without dereferencing a possibly-absent property: under StrictMode
        # that throws, and the outer catch then logs "stamp failed: the property
        # 'Gestalt' cannot be found", which reads like a registry fault rather than
        # the plain fact that the value is not there. Same verdict, honest reason.
        $g = 0
        $gestaltItem = Get-ItemProperty -LiteralPath $virtRoot -ErrorAction SilentlyContinue
        if ($gestaltItem -and ($gestaltItem.PSObject.Properties.Name -contains 'Gestalt')) {
            $g = [int]$gestaltItem.Gestalt
        }
