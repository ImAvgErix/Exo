# Exo - Display scaling / Override via NVAPI + NVTweak + NvCpl registry contract
...
function Stamp-ScalingOverrideAll {
    ...
    Set-ItemProperty $client -Name 'ExoPreferScalingOverride' -Value 1 -Type DWord -Force
    Set-ItemProperty $client -Name 'ExoPreferGpuScaling' -Value 1 -Type DWord -Force
    Set-ItemProperty $client -Name 'ExoPreferNoScaling' -Value 1 -Type DWord -Force
    return $n
}

Write-SLog '=== Exo NvCpl-scale path (no mouse) ==='
$stamped = Stamp-ScalingOverrideAll
Write-SLog "Stamped Override/GPU/NoScale on $stamped device key(s)"

$exe = Get-NvDisplayExe
if (-not $exe) {
    Write-SLog 'FAIL: Exo.NvDisplay.exe missing'
    exit 2
}
