...
# -Experimental is gone. It was declared here and referenced nowhere in the script, while
# the app's "Experimental Steam" toggle sent it on every apply -- a control the user could
# turn on or off with no possible effect on what Steam actually got.

$ErrorActionPreference = 'Stop'
$Script:SteamOptVersion = '1.15.3'
...
function Get-ExoPwsh {
    ...
    throw 'PowerShell 7 is required for Exo Steam helpers. Install Preview: winget install Microsoft.PowerShell.Preview'
}
function Assert-ExoPwsh7 {
    if (Test-ExoIsPwsh7Host) { return }
    $hint = $null
    try { $hint = Get-ExoPwsh } catch { }
    $msg = 'PowerShell 7 is required to run the Steam Optimizer (not Windows PowerShell 5.1). Install Preview: winget install Microsoft.PowerShell.Preview, then re-run from Exo.'
...
function Get-SteamGpuPreferenceTargets([string]$SteamPath) {
    # The Steam CLIENT executables, and nothing else.
    #
    # Deliberately kept free of any library scan: Get-SteamInstalledGameExes calls THIS function
    # to know which paths are the client, so making this one call that one back produces mutual
    # recursion that spins until the process is killed. Snapshot coverage of library games is
    # Get-SteamGpuPreferenceSnapshotTargets' job, below.
    return @(
...
function Get-SteamGpuPreferenceSnapshotTargets([string]$SteamPath) {
    # Everything Apply may stamp, so Repair can put all of it back.
    #
    # The snapshot used to cover only the three client executables while Apply stamps
    # GpuPreference for every discovered library game EXE. Repair replays only what the snapshot
    # captured, so every library stamp was permanent - a machine here carries ~50 that no Repair
    # path could restore or remove, and an Exo uninstall leaves them behind too. AGENTS.md
    # requires Apply to be reversible; docs/TWEAK-AUDIT.md rule 5 forbids write-only mutations.
    $targets = [Collections.Generic.List[string]]::new()
    foreach ($client in @(Get-SteamGpuPreferenceTargets $SteamPath)) { $targets.Add($client) }

    # Resolved exactly the way Set-SteamLibraryGamePolicy resolves them, so the list Repair
    # restores cannot drift from the list Apply writes.
    try {
        foreach ($game in @(Get-SteamInstalledGameExes -SteamPath $SteamPath)) { $targets.Add($game) }
    } catch { }

    # Plus anything Exo stamped previously, so a snapshot taken today still covers a game that
    # has since been uninstalled and can no longer be discovered on disk.
    try {
        $existing = Get-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\DirectX\UserGpuPreferences' -ErrorAction SilentlyContinue
        if ($existing) {
            $steamRootPattern = [regex]::Escape((Join-Path $SteamPath 'steamapps'))
            foreach ($prop in $existing.PSObject.Properties) {
                if ($prop.Name -like 'PS*') { continue }
                if ($prop.Name -match $steamRootPattern) { $targets.Add([string]$prop.Name) }
            }
        }
    } catch { }
...
function Get-SteamGpuPreferenceSnapshot([string]$SteamPath) {
    ...
    $entries = [Collections.Generic.List[hashtable]]::new()
    foreach ($exe in @(Get-SteamGpuPreferenceSnapshotTargets $SteamPath)) {
...
function Get-SteamWindowsRecoverySnapshot([string]$SteamPath) {
    ...
    return @{
        ...
        # The shared machine-wide DSCP switch. Repair only reverts it when no other Exo QoS
        # policy is left, and it needs the user's original value to revert TO - without this
        # the restore branch was unreachable and Repair always deleted it.
        QosDoNotUseNla          = $(
            try {
                $q = Get-ItemProperty -LiteralPath 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\QoS' -ErrorAction SilentlyContinue
                if ($q -and ($q.PSObject.Properties.Name -contains 'Do not use NLA')) { [string]$q.'Do not use NLA' } else { $null }
            } catch { $null }
        )
...
function Write-SteamLaunchCmd([string]$CmdPath, [string]$SteamPath, [string]$HelperPath, [string[]]$CefArgs, [string]$Label) {
    ...
    # Product policy (matches SteamNativeApply.WriteLeanLauncher): CEF + /HIGH only.
    # No background memory-guard process - thrash-free, no dual-cmd conflict with native.
    # HelperPath kept for template write site only; never launched from this cmd.
    $null = $HelperPath
    $cmd = @(
        '@echo off'
        ("rem Exo {0} - quiet CEF + /HIGH only (no background guard process)" -f $Label)
...
function Test-LnkIsSteamClient([string]$LnkPath, [string]$SteamExe, $Wsh) {
    try {
        ...
        # Exo's own launcher is never a Steam shortcut, whatever it currently points at.
        # Defence in depth for the Repair bug above: once Repair had repointed Exo.lnk at
        # steam.exe, the plain "targets steam.exe" test below claimed it, and Apply rewrote
        # the user's Exo entry into a Steam launcher. Checked by the shortcut's own name and
        # by its target, because after the hijack only the name still identified it.
        $lnkName = [IO.Path]::GetFileNameWithoutExtension($LnkPath)
        if ($lnkName -eq 'Exo') { return $false }
        $exoAppRoot = Join-Path $env:LOCALAPPDATA 'Exo'
        if ($target.StartsWith($exoAppRoot, [StringComparison]::OrdinalIgnoreCase)) { return $false }
...
function Set-SteamGpuRouting([string]$SteamPath) {
    ...
    # One definition of "hybrid" for the whole kit; see Test-SteamHybridGpu.
    $hybrid = Test-SteamHybridGpu
    $key = 'HKCU:\Software\Microsoft\DirectX\UserGpuPreferences'
    try {
...
function Set-SteamClientFullscreenOptimizations([string]$SteamPath) {
    # steam.exe ONLY, and clear any stale webhelper stamp - identical to SetClientFso in
    # SteamNativeApply.cs. The FSO shim only engages for a process that presents a fullscreen
    # swapchain: steam.exe does in Big Picture, steamwebhelper.exe is a windowed CEF child and
    # never does, so stamping it is a registry write with no reachable effect.
    #
    # These two used to disagree. The native pass runs first and DELETES the webhelper stamps
    # on purpose; this function ran second and put all three back, so every apply undid the
    # native pass's own documented decision and the machine ended up with the stamps the C#
    # code had just removed. One rule, one owner.
    ...
    try {
        ...
        foreach ($stale in @(Get-SteamGpuPreferenceTargets $SteamPath)) {
            if ($stale -eq $exe) { continue }
            $item = Get-ItemProperty -LiteralPath $fsoKey -ErrorAction SilentlyContinue
            if ($item -and ($item.PSObject.Properties.Name -contains $stale) -and
                ([string]$item.$stale) -match 'DISABLEDXMAXIMIZEDWINDOWEDMODE') {
                Remove-ItemProperty -LiteralPath $fsoKey -Name $stale -Force -ErrorAction SilentlyContinue
            }
        }
        Write-Ok 'Fullscreen Optimizations off on steam.exe (Big Picture); webhelper skipped - windowed'
        return $n
    } catch {
...
function Set-SteamClientDscp {
    ...
    # Without this the policies below are inert on any non-domain network, which is every home
    # PC. Internet and Discord have always set it; Steam never did, so a real machine here held
    # 49 Steam QoS policies that had done nothing since the day they were written while the
    # module reported the work applied. Record the pre-Exo value so Repair can restore it.
    if (Get-Command Enable-ExoQosDscpMarking -ErrorAction SilentlyContinue) {
        $Script:SteamQosNlaPrev = Enable-ExoQosDscpMarking
    }
    $ok = 0
...
# Test-SteamGameExeNameJunk also lives only in SteamDetectCore.ps1 - same reason.

# Get-SteamInstalledGameExes deliberately lives ONLY in SteamDetectCore.ps1, which this
# script dot-sources at the top. There used to be a second copy right here that shadowed
# it, so Apply stamped the set one implementation discovered while Detect measured the
# set the other did. They happened to agree - verified identical, 47 of 47, on a real
# library - but nothing enforced that, and every status bug in this kit so far has been
# Apply and Detect disagreeing about what "done" means. One implementation, no drift.
...
function Set-SteamLibraryGamePolicy {
    ...
    if ($paths.Count -eq 0) {
        ...
        return @{ Paths = 0; Gpu = 0; Fso = 0; FsoOk = 0; Dscp = 0; Names = @() }
    }

    # UserGpuPreferences only decides anything when Windows has more than one adapter to choose
    # between. This loop stamped GpuPreference=2 unconditionally, so a single-GPU desktop - which
    # is most gaming PCs - collected one inert stamp per installed game (51 on a machine here,
    # with one adapter installed). GpuTopology on the C# side has always required both a discrete
    # and an integrated adapter; this is the same test, in the form the kit can run standalone.
    $gpuHybrid = Test-SteamHybridGpu
        ...
        foreach ($exe in $paths) {
            try {
                if ($gpuHybrid) {
                    ...
                    # Clear rather than leave a stamp implying an effect it cannot have.
                    Remove-ItemProperty -LiteralPath $gpuKey -Name $exe -Force -ErrorAction SilentlyContinue
                }
                # Counts paths where the policy was ENFORCED, not paths that were stamped.
                # Save-State derives libraryGamePolicyVerified from this being > 0, and detect
                # reads that marker - so counting only stamps would have made every single-GPU
                # machine (most gaming PCs) unable to report Steam as applied, forever. Clearing
                # an inert stamp is the correct outcome on that topology, not a no-op.
                $gpuN++
            } catch { }
            try {
                # Never dereference a property off a null object: the old shape was
                #   (Get-ItemProperty -Name $exe -ErrorAction SilentlyContinue).$exe
                # which yields nothing when the value is absent, and ".$exe" on nothing
                # throws under Set-StrictMode (Steam.Bootstrap sets it session-wide). The
                # bare catch below swallowed that, so on every path WITHOUT a stale flag -
                # which is most paths on most machines - the loop bailed before $fsoOk++.
                # Same footgun as Get-ExoRegStringOrNull in the Discord kit.
                $cur = ''
                $fsoItem = Get-ItemProperty -LiteralPath $fsoKey -ErrorAction SilentlyContinue
                if ($fsoItem -and ($fsoItem.PSObject.Properties.Name -contains $exe)) {
                    $cur = [string]$fsoItem.$exe
                }
                ...
                # Enforcement, not edits. $fsoCleared counts paths that HAD the flag and
                # lost it, which is 0 on a machine that never had it - the common case.
                # Deriving libraryGamePolicyVerified from that made a clean install read
                # "needs Apply" forever, however many times Apply succeeded.
                $fsoOk++
            } catch { }
        }
    } catch {
...
function Install-WebHelperMemoryGuard([string]$SteamPath) {
    # Reversible companion v4: priority + EcoQoS for Steam host only. Never working-set
    # thrash APIs (freezes CEF). Never touches game processes.
    $sleepGame = 4
    $sleepIdle = 4
...
# Priority + EcoQoS only on non-foreground CEF.
...
function Set-SteamClientPriority([bool]$InGame) {
  ...
  Get-Process -Name 'steamerrorreporter','steamerrorreporter64','streaming_client','steam_monitor' -ErrorAction SilentlyContinue | ForEach-Object {
    try {
      ...
      [void][ExoSteamMemory]::SetPowerThrottled($_.Id, $true)    } catch {}
  }
...
function Invoke-SteamRepair([string]$SteamPath) {
    ...
    # Apply writes up to 80 machine-wide QoS policies and nothing removed them - 49 were left
    # on a real rig, still marking traffic for uninstalled games and surviving an uninstall.
    # Swept by PREFIX, not the recorded name list: the native path never populates that list.
    if (-not (Get-Command Remove-ExoQosPoliciesByPrefix -ErrorAction SilentlyContinue)) {
        $qosLib = Join-Path (Split-Path -Parent $PSScriptRoot) 'lib\Exo.GameBar.ps1'
        if (Test-Path -LiteralPath $qosLib) { . $qosLib }
    }
    if (Get-Command Remove-ExoQosPoliciesByPrefix -ErrorAction SilentlyContinue) {
        try {
            $qosGone = Remove-ExoQosPoliciesByPrefix -Prefixes @('Exo-Steam-DSCP-', 'Exo-SteamGame-DSCP-')
            if ($qosGone -gt 0) {
                $restored++
                Write-Ok "Removed $qosGone Exo Steam QoS policy(ies)"
            } else {
                Write-Ok 'No Exo Steam QoS policies to remove'
            }
        } catch { [void]$failures.Add("QoS cleanup: $($_.Exception.Message)") }
    }

    # 'Do not use NLA' is machine-wide and shared with Internet and Discord, so it is reverted
    # only when no other Exo QoS policy is left behind - the same rule Discord's Repair applies,
    # for the same reason: clearing it would quietly break a module the user did not ask us to
    # touch. Internet's Repair currently reverts it unconditionally; that is fixed separately.
    try {
        $qosRoot = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\QoS'
        $leftover = @(Get-ChildItem -LiteralPath $qosRoot -ErrorAction SilentlyContinue |
            Where-Object { $_.PSChildName -like 'Exo-*' -or $_.PSChildName -like 'Exo *' })
        if ($leftover.Count -gt 0) {
            Write-Ok "Left QoS 'Do not use NLA' in place - $($leftover.Count) other Exo QoS policy(ies) still need it"
        } else {
            # Read through Get-SteamObjectProperty: $recovery is a hashtable and a missing key
            # THROWS under StrictMode, which the old bare $recovery.QosDoNotUseNla did into a
            # catch that set $nlaPrev = $null. Combined with nothing ever WRITING that field,
            # the restore branch below was unreachable: Steam Repair always deleted the shared
            # switch and always claimed it "was not set before Exo", even for a user who had
            # set it themselves. It is now captured by the snapshot, so the claim can be true.
            $nlaPrev = Get-SteamObjectProperty $recovery 'QosDoNotUseNla' $null
            if ($null -eq $nlaPrev -or [string]::IsNullOrWhiteSpace([string]$nlaPrev)) {
                Remove-ItemProperty -LiteralPath $qosRoot -Name 'Do not use NLA' -Force -ErrorAction SilentlyContinue
                Write-Ok "Removed QoS 'Do not use NLA' (no pre-Exo value recorded)"
            } else {
                New-ItemProperty -LiteralPath $qosRoot -Name 'Do not use NLA' -Value ([string]$nlaPrev) `
                    -PropertyType String -Force -ErrorAction SilentlyContinue | Out-Null
                Write-Ok "Restored QoS 'Do not use NLA' to its pre-Exo value ($nlaPrev)"
            }
        }
    } catch { }
    ...
    foreach ($base in (Get-SteamShortcutSearchRoots)) {
        Get-ChildItem -LiteralPath $base -Filter '*.lnk' -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            try {
                ...
                # Anchored to the launcher filename. The old -match 'Steam-Exo|Exo' was
                # unanchored, so it also matched Exo's own Start Menu shortcut and repointed it
                # at steam.exe - after which Apply rewrote it into a Steam launcher for good.
                $target = [string]$sc.TargetPath
                $isExoSteamLauncher = $target -match '(?i)[\\/]Steam-Exo(-Aggressive|-Lean|-Legacy)?\.cmd$'
                # Never touch a shortcut that points into Exo's own install, whatever it is called.
                $exoAppRoot = Join-Path $env:LOCALAPPDATA 'Exo'
                $isExoApp = $target -and $target.StartsWith($exoAppRoot, [StringComparison]::OrdinalIgnoreCase)
                if ($isExoSteamLauncher -and -not $isExoApp) {
                    $sc.TargetPath = $exe
                    $sc.WorkingDirectory = $SteamPath
...
try {
    ...
    try {
        ...
        if (Get-Command Test-SteamMemoryGuardText -ErrorAction SilentlyContinue) {
            ...
            $helperOk = $helperText -match 'Exo\.SteamMemoryGuard' -and
                ...
                $helperText -notmatch 'EmptyWorkingSet\(' -and
                $helperText -notmatch 'SetProcessWorkingSetSize' -and
                $helperText -notmatch 'SoftReclaimWorkingSet'
        }
    } catch { }
    if (-not $helperOk) {
    ...
    # Template is optional (never launched). Report ok either way - do not fail-close apply.
    if ($helperOk) { Add-ExoReport 'background-priority' 'ok' 'template present; not launched (optional)' }
    else { Add-ExoReport 'background-priority' 'ok' 'no guard process (template optional)' }
    $fullPassOk = -not [bool]$Quick
    # Core pack (always required for applied). Memory-guard template is optional.
    # VDF first-run skips are NOT essentials-satisfied: detect requires downloadOptimized/snappyUi true.
    $coreOk = $startupOk -and $windowsQuietOk -and $debloatOk -and $runtimeOk -and $clientHardwareOk -and
        $launchPathOk -and $launcherOk -and $fullPassOk -and $shaderInventoryVerified
    ...
    $state = @{
        ...
        # Both operands must be ENFORCEMENT counts. Fso is "how many stale flags did we
        # clear", which is legitimately 0 on a clean machine; using it here meant a
        # successful Apply still left the row reading "needs Apply", forever.
        libraryGamePolicyVerified = ([int]$libPolicy.Paths -eq 0) -or (
            [int]$libPolicy.Gpu -gt 0 -and [int]$libPolicy.FsoOk -gt 0
        )
        libraryGamePathCount = [int]$libPolicy.Paths
        libraryGameGpuCount  = [int]$libPolicy.Gpu
        libraryGameFsoCount  = [int]$libPolicy.Fso
        libraryGameFsoOkCount = [int]$libPolicy.FsoOk
