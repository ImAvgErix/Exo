...
# -Experimental is gone: it forwarded to a Disc-Optimizer switch that reached one
# Write-Host and changed nothing about the apply.
...
elseif ($PSVersionTable.PSEdition -ne 'Core' -or [int]$PSVersionTable.PSVersion.Major -lt 7) {
    throw 'Exo-Discord-Run requires PowerShell 7. Install Preview: winget install Microsoft.PowerShell.Preview'
}
...
# No DISCOPT_SKIP_MANIFEST: the elevated host verifies the shipped closure before
# execution, so the kit must keep its own integrity checks intact.
