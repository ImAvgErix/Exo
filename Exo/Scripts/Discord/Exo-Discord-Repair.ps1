...
function Remove-ExoDiscordQosPolicies($Recovery) {
    ...
    # The 'Do not use NLA' switch is machine-wide and shared: Apply sets it because
    # DSCP marking is silently dropped off-domain without it, but the Internet
    # optimizer needs the same switch for its own game-traffic policies. Clearing it
    # here would quietly break a module the user did not ask us to touch, so it is
    # only reverted when no other Exo QoS policy is left behind.
    try {
        $leftover = @(Get-ChildItem -LiteralPath $root -ErrorAction SilentlyContinue |
            Where-Object { $_.PSChildName -like 'Exo-*' -or $_.PSChildName -like 'Exo *' })
        if ($leftover.Count -gt 0) {
            Write-RepOk "Left QoS 'Do not use NLA' in place - $($leftover.Count) other Exo QoS policy(ies) still need it"
        } else {
            $before = $null
            try { if ($Recovery) { $before = $Recovery.QosDoNotUseNla } } catch { }
            if ($null -eq $before) {
                Remove-ItemProperty -LiteralPath $root -Name 'Do not use NLA' -Force -ErrorAction SilentlyContinue
                Write-RepOk "Removed QoS 'Do not use NLA' (was not set before Exo)"
            } else {
                New-ItemProperty -LiteralPath $root -Name 'Do not use NLA' -Value ([string]$before) `
                    -PropertyType String -Force -ErrorAction SilentlyContinue | Out-Null
                Write-RepOk "Restored QoS 'Do not use NLA' to its pre-Exo value ($before)"
            }
        }
    } catch {
        Write-RepWarn "QoS 'Do not use NLA': $($_.Exception.Message)"
    }
...
function Remove-ExocordLayer([string]$AppDataRoot) {
    # Undo the Exocord QuickCSS layer without touching anything the user wrote themselves.
    # Apply writes it as a marked block and keeps user CSS below the end marker, so Repair
    # cuts exactly that block back out. A user with their own QuickCSS keeps it; a user who
    # never had any is left with the empty file Equicord shipped, not a deleted one.
    $quickCss = Join-Path $AppDataRoot 'Equicord\settings\quickCss.css'
    if (-not (Test-Path -LiteralPath $quickCss)) { return }
    $beginMarker = '/* ==== EXOCORD LAYER - MANAGED BY EXO, EDITS BELOW THE END MARKER ARE KEPT ==== */'
    $endMarker = '/* ==== END EXOCORD LAYER ==== */'
    try {
        $css = [IO.File]::ReadAllText($quickCss)
        $beginIndex = $css.IndexOf($beginMarker)
        $endIndex = $css.IndexOf($endMarker)
        if ($beginIndex -lt 0 -or $endIndex -lt $beginIndex) { return }
        $before = $css.Substring(0, $beginIndex)
        $after = $css.Substring($endIndex + $endMarker.Length)
        $remaining = ($before + $after).Trim()
        $out = if ($remaining) { $remaining + "`n" } else { '' }
        [IO.File]::WriteAllText($quickCss, $out, [Text.UTF8Encoding]::new($false))
        if ($remaining) {
            Write-RepOk 'Removed Exocord layer (your own QuickCSS kept)'
        } else {
            Write-RepOk 'Removed Exocord layer from quickCss.css'
        }
    } catch {
        Write-RepWarn "Exocord layer removal: $($_.Exception.Message)"
    }
}
...
try {
    ...
    [void](Remove-ExoDiscordQosPolicies $recovery)
