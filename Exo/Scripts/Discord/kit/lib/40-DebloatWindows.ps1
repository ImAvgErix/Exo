...
function Get-DiscordWindowsSnapshot {
    ...
    # Machine-wide QoS switch. Captured here, with every other pre-Exo value,
    # rather than at apply time - by then we may already have set it ourselves.
    $qosNla = $null
    try {
        $qosNla = Get-ExoRegStringOrNull 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\QoS' 'Do not use NLA'
    } catch { }
...
function Merge-DiscordWindowsRecovery($Prior, [hashtable]$Current) {
    ...
    $priorHasNla = ($Prior.PSObject -and ($Prior.PSObject.Properties.Name -contains 'QosDoNotUseNla')) -or
        ($Prior -is [hashtable] -and $Prior.ContainsKey('QosDoNotUseNla'))
    $priorNla = if ($priorHasNla) { $Prior.QosDoNotUseNla } else { $Current.QosDoNotUseNla }
    return @{
        ...
        # First snapshot wins: once recorded, the pre-Exo value must survive every
        # later refresh, or a re-apply would record our own 1 as the original.
        QosDoNotUseNla  = $priorNla
    }
}
...
function Complete-DiscordApplyState([string]$AppDir) {
    ...
    # windowsVerified and debloatVerified were literal $true, and they are exactly the fields
    # every consumer treats as proof: DiscordDetectCore returns false without them, and
    # OptimizerStateService requires both for markerOk. A flag that can only ever be true makes
    # the gate reading it decoration, and let a run that failed a Windows step still record
    # itself as fully verified. Derived now from what this run actually reported.
    #
    # Read from the run's own report rather than re-probing: this module already has three
    # sources of truth for "applied" and a fourth checker would be one more thing to disagree.
    $reportEntries = @(Get-ExoReportEntries)
    $windowsOk = [bool](@($reportEntries | Where-Object { $_ -like 'windows-quiet|ok*' }).Count -gt 0)
    $debloatOk = [bool](@($reportEntries | Where-Object { $_ -like 'debloat|ok*' -or $_ -like 'debloat|skip*' }).Count -gt 0)
    # A 'skip:' entry is legitimate - a real run records relaunch|skip when Discord is left
    # closed on purpose. A '|fail:' never is.
    $anyFailed = [bool](@($reportEntries | Where-Object { $_ -like '*|fail*' }).Count -gt 0)

    $fullOk = [bool]($loaderOk -and $kernelOk -and $windowsOk -and $debloatOk -and (-not $anyFailed))
    if (-not $fullOk) {
        Write-Warn ("Apply record incomplete (equicordLoader=$loaderOk kernel=$kernelOk " +
            "windows=$windowsOk debloat=$debloatOk failedSteps=$anyFailed) - not marking applied")
    }
    Save-DiscordOptState @{
        version           = $Script:DiscOptVersion
        ...
        windowsVerified   = [bool]$windowsOk
        debloatVerified   = [bool]$debloatOk
...
function Get-DiscordManifestCached {
    # Test-Path first. Exo-Discord-Run dot-sources Exo.Common.ps1, which sets
    # Set-StrictMode -Version Latest, and StrictMode is inherited straight through the
    # "&" call into Disc-Optimizer and every kit lib. So this read threw on the FIRST
    # call - the only call that matters for a cache - and the cache could never warm.
    if ((Test-Path Variable:\Script:DiscordManifest) -and $Script:DiscordManifest) {
...
function Get-ExoRegStringOrNull {
    # Reads a registry value without ever dereferencing a property off a null object.
    #
    # The old shape was
    #   (Get-ItemProperty -Path $k -Name $n -ErrorAction SilentlyContinue).$n
    # which returns nothing when the value is absent, and ".$n" on nothing throws
    # "The property '<n>' cannot be found on this object" under Set-StrictMode. That threw
    # INSIDE the try, before the write - so on a machine where the value did not exist yet,
    # which is every machine that needed it, the write never ran and the log said
    # "could not be set". DSCP marking was silently inert off-domain.
...
function Set-ExoQosNlaSwitch {
    # Windows only applies policy-based DSCP marking on a network it considers
    # domain-authenticated, UNLESS this switch is set. On a home network - which is
    # every gaming PC we care about - the marking below is silently dropped without
    # it, so the policy reads back as present and does absolutely nothing on the wire.
    # The Internet optimizer sets the same switch; both need it, and writing 1 twice
    # is idempotent. Returns the value found before we touched it, or $null if unset,
    # so Repair can put it back.
    $qosRoot = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\QoS'
    $before = $null
    try {
        if (Test-Path -LiteralPath $qosRoot) {
            $before = Get-ExoRegStringOrNull $qosRoot 'Do not use NLA'
        } else {
            New-Item -Path $qosRoot -Force -ErrorAction Stop | Out-Null
        }
        New-ItemProperty -LiteralPath $qosRoot -Name 'Do not use NLA' -Value '1' -PropertyType String -Force -ErrorAction Stop | Out-Null
        $now = Get-ExoRegStringOrNull $qosRoot 'Do not use NLA'
        if ([string]$now -eq '1') {
            # Recorded, not just printed. Whether this switch landed decides whether the DSCP
            # policies below do anything at all, so the voice-qos verdict has to be able to see
            # it - it used to be a warning line in the log and nothing else, which let Apply
            # report voice-qos|ok on a run where the marking was inert.
            $Script:ExoQosNlaOk = $true
            Write-Ok 'QoS "Do not use NLA"=1 (without it Windows ignores DSCP on a home network)'
        } else {
            $Script:ExoQosNlaOk = $false
            Write-Warn 'QoS "Do not use NLA" did not stick - DSCP marking may be ignored off-domain'
        }
    } catch {
        $Script:ExoQosNlaOk = $false
        Write-Warn "QoS 'Do not use NLA' could not be set: $($_.Exception.Message) - DSCP marking may be ignored"
    }
    return $before
}
...
function Set-DiscordVoiceQosPolicies {
    ...
    # Must come first: the policies below are inert off-domain without it. The
    # pre-Exo value was already captured by Get-DiscordWindowsSnapshot.
    [void](Set-ExoQosNlaSwitch)
...
# Test-DiscordVoiceQosApplied lived here and was never called - the live detect path is
# Exo-Discord-Detect.ps1 -> Test-DiscOptQosPolicyMap. A second, divergent copy of the same
# check is how a fix lands on the wrong one, so it is deleted rather than kept in sync.
...
function Test-ExoHasDiscreteGpu {
    # True only when Windows has BOTH a discrete and an integrated adapter to choose between.
    #
    # This used to return true whenever any NVIDIA/Radeon/Arc adapter existed, which on a
    # single-GPU desktop is every gaming PC - so Discord.exe got a GpuPreference stamp on
    # machines where Windows has nothing to choose between and the value is inert. The C# side
    # (GpuTopology.IsHybrid) has always required both; this is the same test, kept here because
    # the Discord kit is standalone PowerShell and cannot call into the app.
    #
    # Named as it was because the callers read well ("only when a discrete GPU exists"), and the
    # honest meaning of that in this context is "a discrete GPU the user could be routed away
    # from".
    try {
        $gpus = @(Get-CimInstance Win32_VideoController -ErrorAction SilentlyContinue |
            ForEach-Object { [string]$_.Name } |
            Where-Object { $_ -and ($_ -notmatch '(?i)Microsoft Basic|Hyper-V|Remote|Virtual|Parsec|DisplayLink') })
        if (@($gpus).Count -lt 2) { return $false }
        $discrete = @($gpus | Where-Object { $_ -match '(?i)NVIDIA|GeForce|RTX|GTX|Quadro|Radeon\s+RX|Arc\s*A' }).Count -gt 0
        $integrated = @($gpus | Where-Object { $_ -match '(?i)Intel.*(?:UHD|Iris|HD Graphics)|AMD Radeon\(TM\) Graphics|Radeon Vega' }).Count -gt 0
...
function Test-OpenAsarInstalled([string]$ResourcesDir) {
    ...
    $size = Get-DiscordFileLength $target
...
function Test-ExoHostInstalled([string]$AppDir) {
    ...
    if (-not (Test-Path -LiteralPath $loader -PathType Leaf)) { return $false }
    $len = Get-DiscordFileLength $loader
...
function Ensure-AsarStockBackup([string]$AppDir) {
    ...
    foreach ($src in $candidates) {
        if ((Test-Path -LiteralPath $src -PathType Leaf) -and (Get-DiscordFileLength $src) -gt 1000000) {
...
function Remove-LegacyOpenAsar([string]$AppDir) {
    ...
    if (-not (Test-Path -LiteralPath $bootstrap -PathType Leaf)) { return }
    $len = Get-DiscordFileLength $bootstrap
...
function Install-ExoHost([string]$AppDir) {
    ...
    Write-Step 'Installing Exocord Host (Equicord path - no OpenAsar)...'
    ...
    if (-not (Test-ExoHostInstalled $AppDir)) {
        ...
        Write-Ok 'Exocord Host ready (Equicord loader + host flags)'
    }
}
...
function Get-DiscOptPowerShellExe {
    # PowerShell 7 host for helpers (Preview preferred; any 7.x; never 5.1).
...
function Apply-DiscordProfile([string]$DestPath = '') {
    # Host/chromium lean merge + pure-black AMOLED shell color.
    ...
    # or anything stored in Local Storage (reduced motion, notification channel settings).
    # BACKGROUND_COLOR is set to #000000 on purpose (AMOLED) - detect requires it for that row.
    # Windows quiet (OS toasts) is Apply-WindowsTweaks  -  separate path.
    Write-Step 'Merging host flags + AMOLED shell (audio / tray / startup stay yours)...'
    ...
    if (Test-Path $DestPath) {
        try {
            ...
            foreach ($k in @(
                ...
                'START_MINIMIZED', 'IS_MAXIMIZED', 'IS_MINIMIZED'
            )) {
    ...
    # profiles/discord.json owns the VALUES; this list owns which keys may cross over.
    # chromiumSwitches was missing from it, so the entire switch block in that file was dead -
    # the real switches were a hardcoded duplicate below. Every TTI key was allowed through and
    # then immediately overwritten by a hardcoded literal, so of the eight keys in the profile
    # exactly one (DESKTOP_TTI_SPLASH_USE_WEBP) ever reached the client. Editing that file did
    # nothing, which is the worst possible property for the file that looks like the config.
    $hostOnly = @(
        'SKIP_HOST_UPDATE',
        'debugLogging',
    ...
    $leanSwitches = @{}
    if ($kit.Keys -contains 'chromiumSwitches' -and $kit['chromiumSwitches']) {
        $leanSwitches = ConvertTo-HashtableDeep $kit['chromiumSwitches']
    }
    if ($leanSwitches.Count -eq 0) { throw 'profiles/discord.json has no chromiumSwitches' }
    $existingSw = @{}
    ...
    # Retired switches have to be actively removed. This merge only ever adds, so a machine
    # that ran an older Apply would keep them forever and never see the change.
    #
    # disable-renderer-backgrounding and disable-backgrounding-occluded-windows told Chromium
    # to keep Discord's renderer at full priority and keep painting it while it is invisible.
    # On a machine whose whole purpose is games that is backwards: alt-tab into a fullscreen
    # game and Discord is exactly the window those two flags keep awake, spending CPU and GPU
    # on frames nobody can see, while every other Exo module works to hand that back.
    # disable-background-timer-throttling stays - it keeps JS timers (websocket keepalive,
    # voice signalling) running at full rate, which is the one thing here worth the cost.
    foreach ($retired in @('disable-renderer-backgrounding', 'disable-backgrounding-occluded-windows')) {
        if ($existingSw.Keys -contains $retired) { [void]$existingSw.Remove($retired) }
    }
    ...
    # Pure black chrome - AMOLED + detect row. Theme CSS handles the UI; this is the window shell.
    $merged['BACKGROUND_COLOR'] = '#000000'

    Write-Ok 'Host flags merged (audio / tray / startup untouched; shell AMOLED black)'
    ...
    Write-Ok 'Exocord Host flags applied (SKIP_HOST_UPDATE + chromium lean + TTI + AMOLED shell)'
}
