...
function Get-EqSetting($Object, [string]$Name, $Default = $null) {
    # Reads one key off ConvertFrom-Json output without dereferencing a property that may not
    # be there. settings.json belongs to Equicord, so absence is normal, and under
    # Set-StrictMode an absent property throws instead of yielding $null. Same guarded shape as
    # Get-ExoRegStringOrNull / Get-NvoRegValue / Get-SteamObjectProperty elsewhere in the kit.
...
function Restore-StartMenu {
    ...
    function Test-IsDiscordClientLnk([string]$LnkPath) {
        try {
            ...
            # STABLE ONLY. Set-DiscordLnk rewrites whatever this claims, and it hard-codes
            # %LOCALAPPDATA%\Discord\Update.exe with '--processStart Discord.exe'. So every
            # PTB / Canary / Development shortcut this matched was rewritten to launch STABLE
            # Discord: the Start Menu tile kept the name "Discord PTB" and the position, and
            # opened a different client. The old rules matched them three separate ways - any
            # Update.exe whose arguments merely contained "Discord" (PTB's are
            # '--processStart DiscordPTB.exe'), and the file-name test that explicitly listed
            # canary|ptb|development. Meanwhile the "All Discord builds covered" row stayed
            # green, because it only reads settings.json flags and never checks that a variant
            # is still launchable.
            $variant = '(?i)Discord(PTB|Canary|Development)'
            if (($t -match $variant) -or ($a -match $variant)) { return $false }
            if ($DiscordRoot -and $t) {
                # Anything outside the stable install is not ours to rewrite.
                try {
                    $full = [IO.Path]::GetFullPath($t)
                    $stableRoot = [IO.Path]::GetFullPath($DiscordRoot)
                    $isWscript = $t -match '(?i)wscript\.exe$'
                    if (-not $isWscript -and -not $full.StartsWith($stableRoot, [StringComparison]::OrdinalIgnoreCase)) {
                        return $false
                    }
                } catch { }
            }
            ...
            # Name test narrowed to plain "Discord": the variants are separate products.
            if ($base -match '^(?i)discord$') { return $true }
            return $false
        } catch { return $false }
    }
    ...
    foreach ($desk in @(
        ...
        # "Discord*.lnk" also matches "Discord PTB.lnk" and "Discord Canary.lnk". Exo does not
        # optimize those builds and has no business deleting the user's only way to launch
        # them, so the desktop sweep is stable-only - same rule as the Start Menu one above.
        Get-ChildItem -LiteralPath $desk -Filter 'Discord*.lnk' -Force -ErrorAction SilentlyContinue |
            Where-Object { $_.BaseName -notmatch '(?i)(PTB|Canary|Development)' } |
            ForEach-Object {
...
function Test-KernelOnDisk([string]$AppDir) {
    # Decides whether Apply SKIPS the kernel reinstall, so it must ask the same question
    # Detect judges it by: SHA-256 against the shipped kit, plus the ffmpeg_real.dll backup.
    # A shape check here let a rebuilt kit strand the row red with re-apply unable to fix it.
    $ok = $true
    ...
    # The stock backup must exist, or Repair has nothing to put back.
    $ffmpegReal = Join-Path $AppDir 'ffmpeg_real.dll'
    if (Test-Path $ffmpegReal) {
        Write-Ok 'Kernel on disk: ffmpeg_real.dll (stock backup)'
    } else {
        Write-Warn 'Kernel on disk: ffmpeg_real.dll missing (no stock ffmpeg backup) - reinstalling'
        $ok = $false
    }
    ...
    # Same hash comparison Detect performs, so a shipped kit change forces a reinstall here
    # instead of stranding the row.
    if ($ok -and $KitDir) {
        foreach ($name in @('version.dll', 'ffmpeg.dll')) {
            $shipped = Join-Path $KitDir $name
            $onDisk = Join-Path $AppDir $name
            if (-not (Test-Path -LiteralPath $shipped) -or -not (Test-Path -LiteralPath $onDisk)) { continue }
            try {
                $a = (Get-FileHash -LiteralPath $shipped -Algorithm SHA256).Hash
                $b = (Get-FileHash -LiteralPath $onDisk -Algorithm SHA256).Hash
                if ($a -ne $b) {
                    Write-Warn "Kernel $name differs from the shipped kit - reinstalling"
                    $ok = $false
                }
            } catch { }
        }
...
function Test-DiscOptimizer {
    ...
    if ((Test-Path (Join-Path $EquicordData 'equicord.asar')) -and (Test-Path -LiteralPath $loader -PathType Leaf) -and (Get-DiscordFileLength $loader) -lt 4096) {
    ...
    if (Test-Path $settingsPath) {
        ...
        try {
            # Every read below goes through Get-EqSetting. settings.json is Equicord's file, so
            # any of these keys can simply be absent -- and an absent property THROWS under
            # StrictMode rather than yielding $null. The bare catch made that throw invisible:
            # a settings.json without "enabledThemes" (a fresh install, i.e. exactly the machine
            # being reported on) skipped the plugin line, both theme branches, and both plugin
            # checks, and the summary just quietly lost five lines while reporting success.
            ...
            $themes = Get-EqSetting $s 'enabledThemes'
            if ($themes -and @($themes).Count -gt 0) {
                Write-Ok "Themes on: $(@($themes) -join ', ')"
            } else {
                Write-Warn 'No themes enabled in settings'
            }
            $plugins = Get-EqSetting $s 'plugins'
            $bk = Get-EqSetting $plugins 'BlockKrisp'
            if ($bk -and (Get-EqSetting $bk 'enabled') -eq $true) { Write-Warn 'BlockKrisp enabled - None dropdown may break' }
            else { Write-Ok 'BlockKrisp off (native noise UI)' }
            $dc = Get-EqSetting $plugins 'Declutter'
            if ($dc -and (Get-EqSetting $dc 'removeAudioMenus') -eq $true) { Write-Warn 'Declutter hiding audio menus' }
        } catch {}
    }
}
...
function Write-RunSummary {
    ...
    # The banner used to be the literal string "everything applied successfully", printed
    # unconditionally - twenty lines after this same function had already built a SAFETY bullet
    # saying the mods were rolled back to stock. It also printed after a Quick pass that had
    # just stamped applyStatus='incomplete'. Derive it from what happened.
    #
    # Keyed on the report as well as the rollback flags: the report is the one record that sees
    # every path, including the quiet one Exo itself uses.
    $reportNow = @()
    try { $reportNow = @(Get-ExoReportEntries) } catch { }
    $failedNow = @($reportNow | Where-Object { $_ -like '*|fail*' })
    $rolledBack = [bool]($Script:ModsRolledBack -or $Script:KernelRolledBack)
    $quickPass = $false
    try { $quickPass = [bool](Get-Variable -Name Quick -Scope Script -ErrorAction SilentlyContinue).Value } catch { }

    $headline = if ($rolledBack) { 'DONE - applied, with safety rollbacks (see below)' }
                elseif ($failedNow.Count -gt 0) { 'DONE - applied, but some steps did not take' }
                elseif ($quickPass) { 'DONE - quick pass (a full run is still required)' }
                else { 'DONE - everything applied successfully' }
    $headlineColour = if ($rolledBack -or $failedNow.Count -gt 0 -or $quickPass) { 'Yellow' } else { 'Green' }

    foreach ($entry in $failedNow) {
        $checks.Add("DID NOT TAKE: $($entry -replace '\|fail:?', ' - ')")
    }

    Write-Host ''
    Write-Host '  ========================================' -ForegroundColor DarkGray
    Write-Host "   $headline" -ForegroundColor $headlineColour
    Write-Host '  ========================================' -ForegroundColor DarkGray
