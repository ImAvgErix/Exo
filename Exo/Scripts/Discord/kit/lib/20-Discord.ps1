...
# Discord 1.0.9250+ can leave an unpacked app.asar directory after an
# interrupted update. Treat ASAR paths as files only when measuring them; a
# directory must never reach .Length under StrictMode. Keep the small folder
# shape repairable so Exo can restore it in place without forcing a full
# Discord reinstall.
function Get-DiscordFileLength([string]$Path) {
...
function Test-DiscordUnpackedAsarStub([string]$Path) {
    ...
    for ($i = 1; $i -le $Attempts; $i++) {
        ...
        try {
            ...
            if (Test-Path -LiteralPath $Path) {
                ...
                if (Test-Path -LiteralPath $bak) { Remove-Item -LiteralPath $bak -Recurse -Force -ErrorAction Stop }
            ...
            if ((Test-Path -LiteralPath $Path -PathType Leaf) -and (Get-DiscordFileLength $Path) -eq $Bytes.Length) {
...
function Test-DiscordReady {
    ...
    # A tiny packed loader is ready. An unpacked app.asar directory is a
    # repairable modern-update shape only when the stock shell is still present.
    $packedLength = Get-DiscordFileLength $appAsar
    if ($packedLength -ge 64) { return $true }
    if (Test-DiscordUnpackedAsarStub $appAsar) {
        $stock = Join-Path $resources '_app.asar'
        return (Get-DiscordFileLength $stock) -ge 1000000
    }
    return $false
}
...
function Test-ValidEquicordAsar([string]$Path) {
    return (Test-Path -LiteralPath $Path -PathType Leaf) -and (Get-DiscordFileLength $Path) -gt 1000000
}
...
function Test-EquicordLoaderPatched([string]$AppDir) {
    ...
    $len = Get-DiscordFileLength $appAsar
    # Real Equicord stub is small but not empty/corrupt (1-byte stubs were skipping repair).
    if ($len -lt 64 -or $len -ge 4096) { return $false }
    # Prefer full asar self-check when available (rejects the old padded 500-byte stubs
    # that looked "small enough" but made Discord exit 1 immediately).
    if (Get-Command Test-EquicordLoaderAsarBytes -ErrorAction SilentlyContinue) {
        try {
            $bytes = [IO.File]::ReadAllBytes($appAsar)
            $eq = Join-Path $EquicordData 'equicord.asar'
            return [bool](Test-EquicordLoaderAsarBytes $bytes $eq)
        } catch {
            return $false
        }
    }
...
function Use-StockDiscordRuntime([string]$AppDir) {
    ...
    $stockSource = @($stockBackup, $innerAsar, $equilotBackup) |
        Where-Object {
            (Test-Path -LiteralPath $_ -PathType Leaf) -and
            (Get-DiscordFileLength $_) -gt 1000000
        } |
        Select-Object -First 1

    if (Test-Path -LiteralPath $appAsar -PathType Container) {
        # A failed modern update can unpack app.asar into a directory. Remove
        # only that exact active resource and restore the packed stock shell.
        if ($stockSource) {
            Remove-Item -LiteralPath $appAsar -Recurse -Force -ErrorAction Stop
            Copy-Item -LiteralPath $stockSource -Destination $appAsar -Force
        }
    }

    if ((Test-Path -LiteralPath $appAsar -PathType Leaf) -and (Get-DiscordFileLength $appAsar) -lt 4096) {
        if ($stockSource -and $stockSource -ne $appAsar) {
            Copy-Item -LiteralPath $stockSource -Destination $appAsar -Force
        }
    }

    if ((Test-Path -LiteralPath $innerAsar -PathType Leaf) -and (Get-DiscordFileLength $innerAsar) -lt 500000) {
        foreach ($candidate in @($stockBackup, $equilotBackup)) {
            if ((Test-Path -LiteralPath $candidate -PathType Leaf) -and
                (Get-DiscordFileLength $candidate) -gt 1000000) {
                Copy-Item -LiteralPath $candidate -Destination $innerAsar -Force
                break
            }
        }
    }
...
function Test-DiscordInstallerHealthy {
    ...
    # A size check only catches a truncated DB. The failure that actually strands people
    # is an INCONSISTENT one - full size, structurally fine, and Discord refuses to launch
    # from it. The updater log is the only place that says so, so read it.
    #
    # rCURRENT only, and only if it is newer than the DB. SquirrelSetup.log is deliberately
    # NOT consulted: it is append-only across sessions, so one historical
    # InconsistentInstallerState would mark Discord unhealthy forever and re-run the boot
    # repair on every single Apply - a check that can never pass again is worse than the
    # check that was too lax.
    $updaterLog = Join-Path $AppData 'logs\Discord_updater_rCURRENT.log'
    if (Test-Path -LiteralPath $updaterLog) {
        $dbWritten = (Get-Item -LiteralPath $db).LastWriteTime
        if ((Get-Item -LiteralPath $updaterLog).LastWriteTime -ge $dbWritten) {
            $bad = Select-String -Path $updaterLog -Pattern 'InconsistentInstallerState' -ErrorAction SilentlyContinue |
...
function Repair-DiscordInstallerState {
    ...
    # Actually repair the installer DB, rather than only backing it up.
    #
    # Discord's own updater can leave %LocalAppData%\Discord\installer.db in a state
    # where every launch dies instantly with
    #   fatal: (InconsistentInstallerState) Attempt to install host that is currently
    #   running. current_exe_path: ...\app-<ver>\Discord.exe
    # The client never opens, so it never installs its modules, so Ensure-DiscordBootReady
    # sat in its six-minute wait and Apply failed with "Discord module install timed out -
    # open stock Discord once manually" - advice the user cannot follow, because opening
    # Discord manually hits the same fatal error.
    #
    # Every Discord package ships a pristine installer.db inside app-<ver>. Copying it
    # back over the root one clears the inconsistency; verified on a machine stuck in
    # exactly this state, where Discord went from instant-exit to booting and pulling
    # discord_utils + discord_voice within seconds. Exo-Discord-Repair.ps1 already does
    # this after a staged reinstall; the apply path never learned to.
    try {
        $app = Get-ActiveApp
        if ($app) {
            $pristine = Join-Path $app.FullName 'installer.db'
            $root = Join-Path $DiscordRoot 'installer.db'
            if ((Test-Path -LiteralPath $pristine) -and (Get-Item -LiteralPath $pristine).Length -ge 4096) {
                Copy-Item -LiteralPath $pristine -Destination $root -Force -ErrorAction Stop
                Write-Ok 'Restored Discord installer DB from the installed package'
            }
        }
    } catch {
        Write-Warn "Could not restore Discord installer DB: $($_.Exception.Message)"
    }
}
