...
function Enable-ExoQosDscpMarking {
    <#
    .SYNOPSIS
      Sets the machine-wide switch without which Windows ignores every policy above.
    .DESCRIPTION
      Policy-based DSCP marking is only honoured on a network Windows considers
      domain-authenticated, unless HKLM\SOFTWARE\Policies\Microsoft\Windows\QoS
      "Do not use NLA" = 1. On a home network - every gaming PC - the marking is silently
      dropped, so the policies read back as present and do nothing on the wire.

      The Internet and Discord modules both set this. Steam never did: a real machine here
      carried 49 Exo Steam QoS policies with the switch unset, every one of them inert since
      the day it was written, while Steam's tile reported the work applied.

      Returns the value found before the write ($null when unset) so Repair can put it back.
    #>
    $root = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\QoS'
    $before = $null
    try {
        if (Test-Path -LiteralPath $root) {
            ...
            if ($item -and ($item.PSObject.Properties.Name -contains 'Do not use NLA')) {
                $before = [string]$item.'Do not use NLA'
            }
        } else {
            New-Item -Path $root -Force -ErrorAction Stop | Out-Null
        }
        New-ItemProperty -LiteralPath $root -Name 'Do not use NLA' -Value '1' `
            -PropertyType String -Force -ErrorAction Stop | Out-Null
    } catch { }
    return $before
}

function Remove-ExoQosPoliciesByPrefix {
    <#
    .SYNOPSIS
      Removes every Exo QoS policy whose key name starts with one of the given prefixes.
    .DESCRIPTION
      Repair has to sweep by prefix, not by the list Apply recorded. The native C# apply path
      writes the same policies without ever populating that list, older builds used names the
      current state file has never seen, and the set changes every time the user's library
      does. A machine here had 49 of these left behind with no code path anywhere in the repo
      able to remove them - Remove-ExoGameQosPolicy existed but had zero call sites.
    #>
