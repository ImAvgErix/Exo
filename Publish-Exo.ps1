...
# Optional Authenticode signing. Pass -CodeSignCertificate (pfx path) and
# -CodeSignPassword to sign Exo.exe after the SFX is built. Signing kills the
# SmartScreen "unknown publisher" warning and is required before any store
# distribution. When no certificate is supplied the build ships unsigned and
# the release notes say so - it never blocks a local build.
if ($CodeSignCertificate -and (Test-Path -LiteralPath $CodeSignCertificate)) {
    $signtool = Get-ChildItem 'C:\Program Files (x86)\Windows Kits\10\bin' -Recurse -Filter signtool.exe -ErrorAction SilentlyContinue |
        Sort-Object FullName -Descending | Select-Object -First 1
    if (-not $signtool) {
        Write-Warning "CodeSignCertificate given but signtool.exe not found under Windows Kits; Exo.exe left unsigned."
    } else {
        $args = @('sign', '/fd', 'SHA256', '/tr', 'http://timestamp.digicert.com', '/td', 'SHA256',
            '/f', $CodeSignCertificate)
        if ($CodeSignPassword) { $args += @('/p', $CodeSignPassword) }
        $args += $SfxPath
        & $signtool.FullName @args
        if ($LASTEXITCODE -ne 0) { throw "signtool failed with exit code $LASTEXITCODE" }
        Write-Host "[+] Signed $SfxPath (SHA-256, RFC 3161 timestamp)" -ForegroundColor Green
    }
} elseif ($CodeSignCertificate) {
    throw "CodeSignCertificate not found: $CodeSignCertificate"
} else {
    Write-Host '[ ] Unsigned build (no -CodeSignCertificate); SmartScreen will prompt on first run.' -ForegroundColor DarkYellow
}
